...
class HTML
{
	...
	public function add($content)
	{
		...
		foreach ($content as $content) {
			...
			if (!in_array($content, [null, false, ''], true)) {
				$this->childs[] = $content;
			}
		}
		return $this;
	...
	public function each($callback)
	{
		...
		foreach ($this->childs as &$child) {
			if ($child instanceof self) {
				...
				call_user_func_array($callback, [&$child]);
			}
		}
		unset($child);
		return $this;
	...
	public function addClass($class)
	{
		...
		if (is_array($class)) {
			array_map([$this, __FUNCTION__], $class);
		} else if (is_string($class) && trim($class) && !$this->hasClass($class)) {
	...
	public function toHTML()
	{
		...
		foreach ($this->attrs as $name => $value) {
			...
			if ($value === true) {
				...
				$value = in_array($name, ['href', 'src', 'action', 'itemtype']) ? esc_url($value) : esc_attr($value);
				$attrs[] = $name.'="'.$value.'"';
			}
		}
	...
	public static function makeSelect($name, $value = '', array $options = [], array $groups = [], $multiple = false)
	{
	...
	public static function makeCheckboxGroup($name, array $values = [], array $options = [], $separator = '<br />')
	{
	...
	public static function makeRadioGroup($name, $value = null, array $options = [], $separator = '<br />')
	{
		$fieldset = self::fieldset();
